 ...
 * A prefeitura deseja saber:   
	...
	(x) maior salário; 
	(x) percentual de pessoas com salário até R$100,00.  
...
public class Ex01For {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 3; i++) {
			...
			if (salarios[i] <= 100) {
				qtdSalarioAteCem = qtdSalarioAteCem + 1;
			}
		...
		if (qtdSalarioAteCem > 0) {
			percentual = ((qtdSalarioAteCem / 3) * 100);
		}

		mediaSalario = somaSalarios / salarios.length;
		mediaFilhos = somaFilhos / numFilhos.length;

		System.out.println("\nSalarios até R$ 100,00: " + qtdSalarioAteCem);
